        ...
        <div className="grid gap-5 lg:grid-cols-2 mt-8 lg:mt-16">
          {coursesSecItems.map((item) => (
            ...
            <div className="bg-white p-7 lg:p-10 rounded-[10px]" key={item.id}>
              <div className="">
                <img src={item.img} alt={item.title} width={560} height={266} className="w-full h-full object-cover "/>
              </div>
              ...
              <div className="mt-6">
                <div className="flex items-center flex-wrap gap-2.5">
                  {item.tags.map((tag, index) => (
                    <span className="border border-orange-75 p-2 rounded-md bg-orange-97" key={index}>{tag.tag}</span>
                  ))}
